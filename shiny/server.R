server <- function(input, output, session) {
  ...
  tab_data <- list(
    ...
    consensus_tab =
      list(
        basename = reactive("bin_consensus_"),
    ...
    bin_reps =
      list(
        basename = reactive("BIN-tax_reps_"),
        output = rep_table,
  ...
  observeEvent(input$copy_table, {
    cb(as.data.frame(tab_data[[input$tabs]]$output()[tab_data[[input$tabs]]$current_rows(), ]))
  })
  observeEvent(input$copy_reps, {
    cb(tab_data[[input$tabs]]$output()[tab_data[[input$tabs]]$current_rows(), ][["processid"]], header = FALSE)
  })
  ...
  observeEvent(input$copy_binmates, {
    cb(outdata$binmates, header = FALSE)
  })
